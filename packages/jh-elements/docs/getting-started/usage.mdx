...
Some components will include named slots. Named slots target specific regions of a component to render your HTML. Multiple named slots can be present along with a default slot in some cases. To target a slot, include the slot attribute with the targeted slot name:
