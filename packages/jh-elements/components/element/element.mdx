...
<Meta title="Components/Element" of={stories} />

# Element

[Guidelines](https://jackhenry.design/pages/components/element/)
...
Import the `<jh-element>` component:

```javascript
import '@jack-henry/jh-elements/components/element/element.js';
```

To use `<jh-element>`: *can include details about slot or anything relevant. Keep this short as it is just a code snippet to get the author started.* 
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-element add-relevant-boolean></jh-element>`. While false indicates the attribute does not exist in the DOM: `<jh-element></jh-element>`.
