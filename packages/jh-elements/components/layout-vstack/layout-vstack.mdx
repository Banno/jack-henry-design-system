...
import * as stories from './layout-vstack.stories.js';

<Meta title="Primitives/Layout Vstack" of={stories} />

# Layout Vstack

[Guidelines](https://jackhenry.design/pages/components/layout-vstack/)
...
Import the `<jh-layout-vstack>` component:

```javascript
import '@jack-henry/jh-elements/primitives/layout-vstack/layout-vstack.js';
```

To use `<jh-layout-vstack>`: *can include details about slot or anything relevant. Keep this short as it is just a code snippet to get the author started.* 
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-layout-vstack add-relevant-boolean></jh-layout-vstack>`. While false indicates the attribute does not exist in the DOM: `<jh-layout-vstack></jh-layout-vstack>`.
