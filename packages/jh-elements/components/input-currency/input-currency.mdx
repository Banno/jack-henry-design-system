...
By default (`hide-commas` is `false`), thousands separators are automatically inserted into the value as the user types, and the comma key is blocked from manual entry while typing.

```html
<jh-input-currency value="1000000"></jh-input-currency>
<!-- displays as 1,000,000 -->
```

Set `hide-commas` to `true` to disable automatic comma insertion. When disabled, the user is also permitted to type a comma manually.
...
By default (`hide-decimal` is `false`), the value is formatted with two decimal places as the user types, cash-register style: each digit entered shifts the existing digits left, with the two rightmost digits always representing cents.

```html
<jh-input-currency></jh-input-currency>
<!-- typing 1, 2, 3 in sequence updates the value to 0.01, then 0.12, then 1.23 -->
```

Set `hide-decimal` to `true` to leave the value as typed, without shifting digits or enforcing decimal places:
...
If both `hide-commas` and `hide-decimal` are `false` (the default):

```html
<jh-input-currency></jh-input-currency>
<!-- typing 100000000 updates the value to 1,000,000.00 -->
```
