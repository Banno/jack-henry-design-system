...
[Guidelines](https://jackhenry.design/pages/components/select/)
...
To use `<jh-select>`: *can include details about slot or anything relevant. Keep this short as it is just a code snippet to get the author started.* 
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-select add-relevant-boolean></jh-select>`. While false indicates the attribute does not exist in the DOM: `<jh-select></jh-select>`.
