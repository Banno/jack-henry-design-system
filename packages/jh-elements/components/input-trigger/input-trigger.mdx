...
import * as stories from './input-trigger.stories.js';

<Meta title="Components/Input Trigger" of={stories} />

# Input Trigger

[Guidelines](https://jackhenry.design/pages/components/input-trigger/)
...
Import the `<jh-input-trigger>` component:

```javascript
import '@jack-henry/jh-elements/components/input-trigger/input-trigger.js';
```

To use `<jh-input-trigger>`: *can include details about slot or anything relevant. Keep this short as it is just a code snippet to get the author started.* 
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-input-trigger add-relevant-boolean></jh-input-trigger>`. While false indicates the attribute does not exist in the DOM: `<jh-input-trigger></jh-input-trigger>`.
