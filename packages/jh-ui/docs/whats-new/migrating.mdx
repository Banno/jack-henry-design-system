...
#### jh-button sizes
The button component now supports a new `size="x-small"` option. The existing sizes (small, medium, and large) have been downsized by 8px each. This layout change may reduce the overall height of any components that import `jh-button`, including `jh-notification`.
