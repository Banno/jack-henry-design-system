...
import * as stories from './input-textarea.stories.js';

<Meta title="Components/Input Textarea" of={stories} />
...
The input textarea component provides a multi-line text field that allows users to submit detailed, unstructured, free-form text. Input textarea extends off of the `jh-input` component which allows for the use of its properties, events, methods, and styles. Refer to the [jh-input documentation](?path=/docs/components-input--docs) for detailed information on these features. 

## Usage

Import the `<jh-input-textarea>` component:

```javascript
import '@jack-henry/jh-elements/components/input-textarea/input-textarea.js';
```

To use `<jh-input-textarea>`: 

```html
<jh-input-textarea no-resize></jh-input-textarea>
```

## Auto Grow

Set `auto-grow` to allow the textarea to automatically grow to match the height of its content.

Unless `no-resize` is set, users can still utilize the native resizer to momentarily override the dimensions of the textarea. However, changes to the content will cause the textarea to “snap” back to the height of the content.

## Dependencies
The input textarea component makes use of required compositional dependencies.
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-input-textarea auto-grow></jh-input-textarea>`. While false indicates the attribute does not exist in the DOM: `<jh-input-textarea></jh-input-textarea>`.
