...
Properties can be used alone or in combination with the left and right slots. 
- Use the left and right slots to insert supplementary content such as icons or actions 
on either side of the list item.
- The default slot and properties (`primary-text`, `secondary-text`) are mutually exclusive. 
If content is provided in the default slot, the properties will not be displayed. 
