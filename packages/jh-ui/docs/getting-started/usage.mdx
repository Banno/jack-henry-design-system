...
<<<<<<< HEAD
The exposed styling hooks are listed under the CSS custom properties section of each component API table. To override a component's style, you'll need to apply the corresponding styling hook to the component instance:
=======
...
>>>>>>> c724c6b52d31c1ea751d7de1ac7b07cdea9e6852
...
The easiest way to self-host is to import the `fonts.css` file from jh-core with all the required font declarations into your main css or js file.
...
Some components will include named slots. Named slots target specific regions of a component to render your HTML. Multiple named slots can be present along with a default slot in some cases. To target a named slot, include the `slot` attribute with the targeted slot name on your element:
