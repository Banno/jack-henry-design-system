...
To use `jh-list-group`, place jh-list-items into the default slot. Use the label attribute to provide an optional subheader. The subheader is set as a small medium-weight label with more space above than below, so it reads as belonging to the items under it.
