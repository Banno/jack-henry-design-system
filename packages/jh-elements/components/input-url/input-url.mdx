...
import * as stories from './input-url.stories.js';

<Meta title="Components/Input Url" of={stories} />
...
The input url component provides a single-line text field for capturing URLs. Input url extends off of the `jh-input` component which allows for the use of its properties, events, slots, methods, and styles. Refer to the [jh-input documentation](?path=/docs/components-input--docs) for detailed information on these features. 

## Usage

Import the `<jh-input-url>` component:
...
## Dependencies
The input url component makes use of required compositional dependencies.
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-input-url required></jh-input-url>`. While false indicates the attribute does not exist in the DOM: `<jh-input-url></jh-input-url>`.
