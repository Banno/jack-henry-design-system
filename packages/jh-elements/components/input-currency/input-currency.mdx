...
A `+` or `-` can be typed to indicate a positive or negative value, such as for deposits and withdrawals. A sign is only accepted as the first character of the value; it cannot be typed anywhere else, and only one sign is permitted at a time.
