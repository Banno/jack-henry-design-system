...
import * as stories from './input-currency.stories.js';

<Meta title="Components/Input Currency" of={stories} />

# Input Currency

[Guidelines](https://jackhenry.design/v2/components/input-currency/)

<hr />

The input currency component provides a single-line text field for capturing currency values. Input currency extends off of the `jh-input` component which allows for the use of its properties, events, slots, methods, and styles. Refer to the [jh-input documentation](?path=/docs/components-input--docs) for detailed information on these features.

## Usage

Import the `<jh-input-currency>` component:

```javascript
import '@jack-henry/jh-elements/components/input-currency/input-currency.js';
```

To use `<jh-input-currency>`: 
...
Use the `prefix` and `suffix` properties to display static text alongside the input value, such as a currency symbol or currency code.

```html
<jh-input-currency prefix="€" value="1000"></jh-input-currency>
<jh-input-currency suffix="USD" value="1000"></jh-input-currency>
```

Prefix and suffix text is decorative only; it is not included in the input's value.

## Formatting

The input currency component provides the following properties to control automatic value formatting: `hide-commas` and `hide-decimal`. Both are `false` by default, so comma and decimal formatting are applied out of the box.

By default, the value is formatted with two decimal places as the user types, cash-register style: each digit entered shifts the existing digits left, with the two rightmost digits always representing cents. Once the value reaches the thousands, comma separators are added automatically.

Typing **1**, **2**, **3**, **4**, **5**, **6** in sequence updates the value to **0.01**, then **0.12**, **1.23**, **12.34**, **123.45**, and finally **1,234.56**.

Pasted values are never reformatted, regardless of `hide-commas` or `hide-decimal`; no digit shifting, decimal enforcement, or comma insertion is applied, so authors are responsible for validating pasted content.

### hide-decimal

Set the `hide-decimal` property to leave the value as typed, without shifting digits or enforcing decimal places:

```html
<jh-input-currency hide-decimal value="1000"></jh-input-currency>
```

Typing is not reformatted; the value displays exactly as entered.

When `hide-decimals` is true, users cannot enter `.`. 

### hide-commas

Set the `hide-commas` property to disable automatic comma insertion. Commas can never be typed manually, regardless of this setting.

```html
<jh-input-currency hide-commas value="1000000"></jh-input-currency>
```
...
`+`, `-`, `(`, and `)` can be typed in the value to indicate a positive or negative value, such as for deposits and withdrawals, or accounting-style negatives like `(123)`.

## Dependencies
The input currency component makes use of required compositional dependencies.
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-input-currency required></jh-input-currency>`. While false indicates the attribute does not exist in the DOM: `<jh-input-currency></jh-input-currency>`.
