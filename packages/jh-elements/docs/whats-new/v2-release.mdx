...
The Design System team has been heavily focused on delivering the next iteration of the Forge Design System, with V2 representing a significant step 
...
All components now officially extend from `JhElement`. This new underlying base class drives better code reuse and allows dependencies to be shared efficiently across the library. The base class introduces a new event API, and several `event.detail` property paths have changed. Please refer to the [JhElement Base Class Migration Guide](?path=/docs/what-s-new-migrating--docs#jhelement-base-class) for the updated paths. Additional properties will be added to event payloads soon. 
Please note that `JhElement` serves strictly as a foundational class and is **not** intended to be implemented as a standalone component.
