...
The input number component provides a single-line text field specifically for incrementable numeric values (such as quantities or amounts) and includes stepper buttons to decrement and increment values. It should not be used for numeric identifiers like ZIP codes, Social Security numbers (SSN), or credit card numbers.

Input number extends off of the `jh-input` component which allows for the use of its properties, events, slots, methods, and styles. Refer to the [jh-input documentation](?path=/docs/components-input--docs) for detailed information on these features.
