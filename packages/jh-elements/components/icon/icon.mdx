...
Icon color is inherited by the slotted SVG via `--jh-icon-color-fill`, so slotted markup should not set its own
`fill`. The component does not set `stroke`, so icons drawn with strokes rather than fills will not pick up the
icon color.
