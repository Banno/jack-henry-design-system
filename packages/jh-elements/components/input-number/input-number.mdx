...
import * as stories from './input-number.stories.js';

<Meta title="Components/Input Number" of={stories} />

# Input Number

[Guidelines](https://jackhenry.design/v2/components/input-number/)

<hr />

The input number component provides a single-line text field for numeric input and includes stepper buttons to decrement and increment values. Input number extends off of the `jh-input` component which allows for the use of its properties, events, slots, methods, and styles. Refer to the [jh-input documentation](?path=/docs/components-input--docs) for detailed information on these features.

## Usage

Import the `<jh-input-number>` component:

```javascript
import '@jack-henry/jh-elements/components/input-number/input-number.js';
```

To use `<jh-input-number>`:

```html
<jh-input-number
	label="Quantity"
	helper-text="Enter a number between 1 and 10"
	min="1"
	max="10"
	accessible-label-decrement-stepper="Decrease quantity"
	accessible-label-increment-stepper="Increase quantity"
></jh-input-number>
```

## Dependencies
The input number component makes use of both required compositional and suggested content related dependencies.
...
- `<jh-button>`. [Button component documentation](?path=/docs/components-button--docs).
- `<jh-icon-minus>` and `<jh-icon-plus>`. [Iconography documentation](?path=/docs/iconography-overview--docs).
...
In addition to the accessibility features provided by `jh-input`, `jh-input-number` provides the following:

- To satisfy [WCAG 2.1.1 Keyboard](https://www.w3.org/WAI/WCAG22/Understanding/keyboard.html),  the input field replicates native `type="number"` behavior, ensuring all functions are operable via keyboard.
...
- To satisfy [WCAG 4.1.2 Name, Role, Value](https://www.w3.org/WAI/WCAG22/Understanding/name-role-value), set the `accessible-label` attribute(s) to generate an aria-label when a visible label cannot be used.

## Component API

For boolean attributes, true indicates that the attribute has been set without a value: `<jh-input-number required></jh-input-number>`. While false indicates the attribute does not exist in the DOM: `<jh-input-number></jh-input-number>`.
