...
| --- | --- | --- |
| `--jh-badge-border-radius` | The roundness of the badge's corners | `--jh-border-radius-pill` |
| `--jh-badge-color-background-enabled` | The badge's background color | `--jh-color-content-negative-enabled` |
...
| Style hook | What it controls | Default value |
| --- | --- | --- |
| `--jh-badge-color-text-enabled` | The color of the count text | `--jh-color-content-on-negative-enabled` |
...
Because a truncated count like `99+` can be trickier for assistive technology to interpret than the exact number, take a moment to think through how that value is announced when you're placing a badge in your experience.
...
- Choose the dot-only variant when an exact count adds noise rather than clarity, and the count variant when the specific number matters to the person using it.
...
| --- | --- | --- | --- |
| `count` | `string` | — | Sets the number to show inside the badge. If not set, the badge renders as a dot. |
| `max-count` | `number` | `99` | Sets the maximum count to show. Appends a `+` to the badge's text once the count exceeds this value. |

### CSS custom properties

| CSS custom property | What it controls | Default value |
| --- | --- | --- |
| `--jh-badge-border-radius` | The badge's border radius. | `--jh-border-radius-pill` |
| `--jh-badge-color-background-enabled` | The badge's background color. | `--jh-color-content-negative-enabled` |
| `--jh-badge-color-text-enabled` | The badge's text color. | `--jh-color-content-on-negative-enabled` |
