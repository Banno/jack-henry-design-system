...
<Meta title="Guides/Extending Components" />

# Extending Components

While our design system provides a comprehensive set of web components for most use cases, there may be instances where additional features are required that go beyond the scope of the existing components.

When specialized components are required, we suggest extending our existing components. By leveraging our existing components, you utilize built-in properties, methods, styles, and accessibility features, ensuring seamless integration while saving significant development time.
...
JhInput.register('my-input', MyInput);
```