...
A non-editable single select dropdown component that displays a list of options. Coming soon is the `jh-datasets` package. Once released, the `jh-select` component will integrate seamlessly with the `jh-datasets` package, allowing you to quickly populate the menu with pre-formatted datasets like US states without manually structuring the data.
