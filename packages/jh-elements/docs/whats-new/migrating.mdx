...
`JhElement` introduces a new event API. The payload structures (event.detail) for our component events have been updated, and event listeners will need to use the new property paths below.
...
<table>
  ...
  <tbody>
    <tr>
      <td>Checkbox</td>
      <td>`jh-change`</td>
      <td>`e.detail.checked`</td>
      <td>`e.detail.state.checked`</td>
    </tr>
    <tr>
