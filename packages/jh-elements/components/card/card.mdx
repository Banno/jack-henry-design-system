...
- For multiple dividers with varying inset requirements, use the divider-inset attribute in conjunction with the `--jh-divider-space-inset` component token. Note that the divider-inset attribute will override the component token where present.
