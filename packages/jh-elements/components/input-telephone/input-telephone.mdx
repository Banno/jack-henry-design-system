...
import * as stories from './input-telephone.stories.js';

<Meta title="Components/Input Telephone" of={stories} />
...
The input telephone component provides a single-line text field for capturing telephone numbers. Input telephone extends off of the `jh-input` component which allows for the use of its properties, events, slots, methods, and styles. Refer to the [jh-input documentation](?path=/docs/components-input--docs) for detailed information on these features. 

## Usage

Import the `<jh-input-telephone>` component:

```javascript
import '@jack-henry/jh-elements/components/input-telephone/input-telephone.js';
```

To use `<jh-input-telephone>`: 
...
## Dependencies
The input telephone component makes use of required compositional dependencies.
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-input-telephone required></jh-input-telephone>`. While false indicates the attribute does not exist in the DOM: `<jh-input-telephone></jh-input-telephone>`.
