...
The input currency component provides the following properties to control automatic value formatting: `show-commas` and `show-decimal`.

### show-commas

When `show-commas` is enabled (true by default), thousands separators are automatically inserted into the value as the user types, and the comma key is blocked from manual entry while typing.
...
Set `show-commas` to `false` to disable automatic comma insertion. When disabled, the user is also prevented from typing a comma manually.

### show-decimal

When `show-decimal` is enabled, the value is rounded to two decimal places when the input loses focus.

```html
<jh-input-currency value="1000"></jh-input-currency>
<!-- after blur, displays as 1,000.00 -->
```

Set `show-decimal` to `false` to leave the value as entered, without appending or enforcing decimal places on blur:

```html
<jh-input-currency show-decimal show-commas="false" value="1000"></jh-input-currency>
<!-- after blur, displays as 1000.00 -->
```

If both `show-commas` and `show-decimal` are true:

```html
<jh-input-currency show-decimal value="1000"></jh-input-currency>
<!-- after blur, displays as 1,000.00 -->
```
