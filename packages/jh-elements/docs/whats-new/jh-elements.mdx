...
A non-editable single select dropdown component that displays a list of options. The `jh-select` component integrates seamlessly with the `jh-datasets` package (coming soon), allowing you to quickly populate the menu with pre-formatted datasets like US states without manually structuring the data.
