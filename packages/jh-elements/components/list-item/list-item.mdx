...
- Use the default slot to provide custom content. The default slot can be used alone 
or in combination with the left and right slots.
- Use the left and right slots to insert supplementary content such as icons or actions 
on either side of the list item.
- Use the `primary-text` and `secondary-text` properties to display formatted text. 
These properties can be used alone or in combination with the left and right slots. 
- Use the `primary-metadata` and `secondary-metadata` properties to display 
metadata aligned to the right of the text content. Metadata properties can be used in combination 
with `primary-text` and `secondary-text` and in combination with the left and right slots.
- The default slot and properties (`primary-text`, `secondary-text`, `primary-metadata`, `secondary-metadata`) 
are mutually exclusive. If content is provided in the default slot, the properties will not be displayed. 
...
- Interactive list items have states such as hover, focus, active, selected and disabled.
- Hover and active are drawn as a translucent state layer (`--jh-color-state-hover` / `-active`), focus shows the outline only, and selected uses a tinted fill with no left bar; the bar is opt-in via `--jh-list-item-border-selected-width` for navigation lists.
