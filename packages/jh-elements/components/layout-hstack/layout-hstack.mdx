...
import * as stories from './layout-hstack.stories.js';

<Meta title="Primitives/Layout Hstack" of={stories} />

# Layout Hstack

[Guidelines](https://jackhenry.design/pages/components/layout-hstack/)
...
Import the `<jh-layout-hstack>` component:

```javascript
import '@jack-henry/jh-elements/primitives/layout-hstack/layout-hstack.js';
```

To use `<jh-layout-hstack>`: *can include details about slot or anything relevant. Keep this short as it is just a code snippet to get the author started.* 
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-layout-hstack add-relevant-boolean></jh-layout-hstack>`. While false indicates the attribute does not exist in the DOM: `<jh-layout-hstack></jh-layout-hstack>`.
