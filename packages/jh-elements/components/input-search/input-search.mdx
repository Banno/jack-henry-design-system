...
<Meta title="Components/Input Search" of={stories} />
...
The input search component provides a single-line text field for search queries. Input search extends off of the `jh-input` component which allows for the use of its properties, events, slots, methods, and styles. Refer to the [jh-input documentation](?path=/docs/components-input--docs) for detailed information on these features. 

## Usage

Import the `<jh-input-search>` component:

```javascript
import '@jack-henry/jh-elements/components/input-search/input-search.js';
```

To use `<jh-input-search>`: 
...
## Dependencies
The input search component makes use of both required compositional and suggested content related dependencies.
...
- `<jh-icon-magnifying-glass>`. [Iconography documentation](?path=/docs/iconography-overview--docs).
...
For boolean attributes, true indicates that the attribute has been set without a value: `<jh-input-search show-clear-button></jh-input-search>`. While false indicates the attribute does not exist in the DOM: `<jh-input-search></jh-input-search>`.
