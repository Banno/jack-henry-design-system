...
**Warning!** This page is continuously changing. We may add or remove scope as needed. All v2 pages can be viewed [here](https://release-v2--60b69cf9d763ec0039a5ac96.chromatic.com/?path=/docs/welcome-about-jh--docs)
...
- **Typography**: The `font` composite token has been broken out into `font-family`, `font-size`, `font-weight`, and `line-height` tokens.
- **Borders**: The `border` composite token has been broken out into `border-width`, `border-style`, and `border-color` tokens.
- **Shadow**: The `box-shadow` composite token cannot be broken out into individual properties in CSS, as it is a single property that defines multiple values. 
This means that the usage and override method remains the same as in v1.

### Shadow & Overlay Tokens

Shadow and overlay tokens are now alias tokens that make use of the new black and white alpha color families.
Moreover, shadow tokens are now defined as composite tokens as per the DTCG specification.
...
We've made significant updates to our documentation including new sections for Accessibility and a guide for extending design system components.
