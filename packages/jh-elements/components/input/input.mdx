...
**Event Handling**: The `jh-input` event is triggered on input. The event details include both the formatted and unformatted input values, and can be accessed via `e.detail.value` and `e.detail.rawValue`, respectively.
