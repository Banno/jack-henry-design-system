...
3. **One existing page in `docs/components/`** (e.g. `badge.md`) — read it to match established formatting conventions already in use in this repo: frontmatter shape (`title`, `description`), table header style, the closing "Feedback" boilerplate (links to `https://github.com/Banno/jack-henry-design-system/issues` and `/discussions`), and general prose tone. Do not copy its facts for a different component — only its structural/formatting conventions.
4. **`https://jackhenry.design/v2/components/<slug>`** and the Chromatic/Storybook instance (currently `https://main--68f8e6a25b256d0ef89b13e6.chromatic.com/`) — fetch these (WebFetch, or the built-in/Chrome browser if WebFetch can't render the SPA) purely to see which of the section types below are used for this component and in what order. If both are unreachable, fall back to the template in Step 2 as-is and note in your final report that you couldn't cross-check structure.
