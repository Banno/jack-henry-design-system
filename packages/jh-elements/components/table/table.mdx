...
- **Controlled:** A specific `width` can be set on the header cells to control the column widths. It is strongly recommended to use percentages (%) for the column widths to ensure the table remains responsive. The table will expand to fill the width of its container.
- **Wide/Scrollable:** For wide tables, horizontal scrolling can be enabled by setting the `scrollable` attribute on the table. To set the width of the header cells, set `min-width` on the cells. 
